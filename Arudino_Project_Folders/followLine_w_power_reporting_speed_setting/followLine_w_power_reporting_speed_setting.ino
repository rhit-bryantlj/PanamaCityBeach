...
void setup() {
  ...
  pinMode(34, INPUT); //switch 1
  pinMode(35, INPUT); //switch 2
...
void reset_servo_pos(){
  while(1){
    ...
    int sw2 = digitalRead(35); // read sw2
    //Serial.println(sw2);
    if(sw2 == 0){
      return;
    }
...
void loop() {
  while(BT_started == 0){
    ...
    readBluetooth();
    int sw1 = digitalRead(34); //switch 1
    //Serial.println(sw1);
    if(sw1 == 0){
      reset_servo_pos();
    }
...
void followLine(HUSKYLENSResult result){//Put back result
  ...
  if (result.command == COMMAND_RETURN_ARROW){ // check to see if result returned is an arrow following ling
    ...
    if(abs(result.xTarget-result.xOrigin) < 30 && abs(xDiff) < 120){
      theta = map(xDiff, -80, 80, 120, 60);
      motorSpeed = straightSpeed;
    } else{
...
void readBluetooth(){
  ...
  if (SerialBT.available()) {
    char buffer[50];
    int numBytes = SerialBT.readBytes(buffer, 3);
    //char byte2 = SerialBT.read();
   // char byte3 = SerialBT.read();
    String str_speed = "";
    str_speed += buffer[0];
    str_speed += buffer[1];
   // str_speed += byte3;
    if(buffer[0] == 'S' || buffer[0] == 'E'){
      switch(buffer[0]){
       case 'S':
        BT_started = 1;
        BT_ended = 0;
      ...
      newSpeed = str_speed.toInt();
      if(buffer[2] == 's'){
        straightSpeed = newSpeed;
      } else if(buffer[2] == 't'){
        turnSpeed = newSpeed;
      }
    }
    Serial.print("\nbytes returned "); Serial.print(buffer[0]);Serial.print(buffer[1]); Serial.print(buffer[2]);
    Serial.print("\nString created: "); Serial.print(str_speed);
    Serial.print("\nNew Speed: "); Serial.print(newSpeed);
 }
